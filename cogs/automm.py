...
class AutoMMDropdownView(discord.ui.View):
    ...
    async def _on_select(self, interaction: discord.Interaction):
        ...
        bank = get_bank_name(interaction.guild_id)
        embed = discord.Embed(
        ...
        mm_role_id = db.get_config(str(interaction.guild_id), "mm_role")
        ...
        if cog:
            db.update_automm_session(self.channel_id, state="waiting_user2")
...
class RoleSelectView(discord.ui.View):
    ...
    def _make_status_embed(self, guild_id):
        bank = get_bank_name(guild_id)
        lines = []
    ...
    async def _try_advance(self, interaction: discord.Interaction):
        ...
        if len(self._selections) == 2:
            ...
            if roles.count("sender") == 1 and roles.count("receiver") == 1:
                ...
                db.update_automm_session(self.channel_id, sender_id=sender_id, receiver_id=receiver_id, state="waiting_method")
                self.stop()
    ...
    async def _show_method_select(self, interaction: discord.Interaction):
        session = db.get_automm_session(self.channel_id)
    ...
    async def be_sender(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if self._selections.get(other) == "sender":
            await interaction.response.send_message("The other trader already chose Sender. Please choose Receiver.", ephemeral=True)
            return
        self._selections[uid] = "sender"
        embed = self._make_status_embed(interaction.guild_id)
        if not await self._try_advance(interaction):
    ...
    async def be_receiver(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if self._selections.get(other) == "receiver":
            await interaction.response.send_message("The other trader already chose Receiver. Please choose Sender.", ephemeral=True)
            return
    ...
    async def reset(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        db.update_automm_session(self.channel_id, sender_id=None, receiver_id=None)
        embed = self._make_status_embed(interaction.guild_id)
        await interaction.response.edit_message(embed=embed, view=self)
...
class MethodSelectView(discord.ui.View):
    ...
    async def _method_chosen(self, interaction: discord.Interaction):
        ...
        db.update_automm_session(self.channel_id, payment_method=method, state="waiting_done")
        self.stop()
...
class DMConfirmView(discord.ui.View):
    ...
    async def confirm_sent(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        db.update_automm_session(self.channel_id, state="waiting_trade_done")
        self.stop()

        guild = interaction.client.get_guild(self.guild_id)
        bank = get_bank_name(self.guild_id)
        support_mention = get_support_channel_mention(guild, self.guild_id)
...
class AutoMM(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self._waiting_for_user2    = set()
        # Keyed by user_id str — set by PanelServiceSelectView before ticket creation
    ...
    async def _start_session(self, channel: discord.TextChannel, guild: discord.Guild, opener: discord.Member):
        # Check if user already chose a service from the panel dropdown
        service = self._preselected_service.pop(str(opener.id), None)

        db.create_automm_session(str(channel.id), str(guild.id), str(opener.id))
        ...
        if service:
            # Service already chosen — skip dropdown, go straight to waiting_user2
            db.update_automm_session(str(channel.id), state="waiting_user2")
            self._waiting_for_user2.add(str(channel.id))

            mm_role_id = db.get_config(str(guild.id), "mm_role")
            ping = f"<@&{mm_role_id}>" if mm_role_id else "@MM Staff"
            bank = get_bank_name(guild.id)
            await channel.send(
            ...
            # Fallback: show dropdown inside the ticket
            db.update_automm_session(str(channel.id), state="waiting_dropdown")
            dropdown_label = cfg.get("automm_dropdown_label") or "What service do you need?"
            options = _parse_dropdown_options(str(guild.id))
    ...
    async def on_message(self, message: discord.Message):
        ...
        if content.lower() == ".automm":
            session = db.get_automm_session_by_sender(str(message.guild.id), str(message.author.id))
            if not session:
                return
            try:
            ...
            bank = get_bank_name(message.guild.id)
            embed = discord.Embed(
        ...
        if content.lower() == ".done":
            session = db.get_automm_session(channel_id)
        ...
        if content.lower() == "done":
            ...
            if session["user1_ready"] and session["user2_ready"]:
                db.update_automm_session(channel_id, state="waiting_automm")
        ...
        self._waiting_for_user2.discard(channel_id)
        db.update_automm_session(channel_id, user2_id=str(target_member.id), state="waiting_roles")

        await message.channel.set_permissions(target_member, read_messages=True, send_messages=True)
        db.add_ticket_user(channel_id, str(target_member.id))
    ...
    async def on_ticket_opened(self, channel: discord.TextChannel, guild: discord.Guild, opener: discord.Member):
        if db.get_ticket(str(channel.id)):
            ticket = db.get_ticket(str(channel.id))
            if ticket.get("ticket_type") == "automm":
